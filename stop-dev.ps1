...
if ($nodeProcesses) {
    Write-Host "   🔍 Encontrados $($nodeProcesses.Count) proceso(s) de Node.js" -ForegroundColor Cyan
    
    foreach ($proc in $nodeProcesses) {
        try {
            Stop-Process -Id $proc.Id -Force
            Write-Host "   ✅ Detenido proceso Node.js (PID: $($proc.Id))" -ForegroundColor Green
        } catch {
            Write-Host "   ⚠️  No se pudo detener proceso $($proc.Id)" -ForegroundColor Yellow
        }
    }
} else {
    ...
    if ($tcpConnection) {
        ...
        if ($process) {
            try {
                ...
                Write-Host "   ✅ Liberado puerto $port (PID: $processId)" -ForegroundColor Green
                $processesKilled++
            } catch {
...
if ($processesKilled -eq 0) {
    Write-Host "   ℹ️  Puertos 4000 y 4200 ya están libres" -ForegroundColor Gray
}
...
if ($remainingNode) {
    Write-Host "   ⚠️  Aún quedan $($remainingNode.Count) proceso(s) de Node.js" -ForegroundColor Yellow
    Write-Host "   Puede que algunos sean de otras aplicaciones" -ForegroundColor Gray
} else {
    Write-Host "   ✅ Todos los procesos detenidos correctamente" -ForegroundColor Green
}
...
if ($port4000) {
    Write-Host "⚠️  Aún ocupado" -ForegroundColor Yellow
} else {
...
Write-Host ""
Write-Host "⚙️  Para iniciar nuevamente: " -NoNewline -ForegroundColor Yellow
