...
if (-not $pgService) {
    Write-Host "   ⚠️  PostgreSQL no encontrado como servicio" -ForegroundColor Red
...
if (-not (Test-Path $BackendPath)) {
    Write-Host "   ❌ No se encuentra el directorio del backend: $BackendPath" -ForegroundColor Red
    exit 1
}

if (-not (Test-Path $FrontendPath)) {
    Write-Host "   ❌ No se encuentra el directorio del frontend: $FrontendPath" -ForegroundColor Red
    exit 1
}
...
# ============================================
Write-Host "`n[3/6] Verificando compilación del backend..." -ForegroundColor Yellow

$DistPath = Join-Path $BackendPath "dist"

if (-not (Test-Path $DistPath)) {
    Write-Host "   ⚠️  Backend no compilado. Compilando ahora..." -ForegroundColor Yellow
    ...
    Write-Host "   ✅ Backend ya compilado (carpeta dist/ existe)" -ForegroundColor Green
}
...
try {
    ...
    Write-Host "   ✅ Backend iniciado correctamente en $BackendUrl" -ForegroundColor Green
} catch {
    Write-Host "   ⚠️  Backend iniciando... (puede tardar unos segundos más)" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "⚙️  Para detener todo, ejecuta: " -NoNewline -ForegroundColor Yellow
...
Write-Host ""
Write-Host "⌨️  Presiona Ctrl+C en las ventanas de PowerShell para detener cada servicio" -ForegroundColor Gray
Write-Host ""
