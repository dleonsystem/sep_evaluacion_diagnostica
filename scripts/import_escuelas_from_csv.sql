-- Importación de datos a la tabla escuelas desde CSV
-- Fuente: data/ESCUELAS_VLP.csv
-- Uso con psql (cliente):
--   psql -h <host> -U <user> -d <db> -f scripts/import_escuelas_from_csv.sql
...
-- 3) Deduplicar por CCT (preferir turno 1 si existe)
WITH ranked AS (
  SELECT *,
         ROW_NUMBER() OVER (
...
-- Notas:
-- - Este script asume que los IDs de turno, nivel, entidad y ciclo ya existen en sus catálogos.
-- - Los correos vacíos se convierten a NULL para cumplir el CHECK de formato.
-- - Si hay múltiples filas por CCT, se prioriza la de turno 1.
